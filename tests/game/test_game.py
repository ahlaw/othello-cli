...
def test_raises_exception_on_illegal_pass(new_game):
    with pytest.raises(InvalidMoveError):
        new_game.apply_move(Move.pass_turn())


def test_game_not_over_at_start(new_game):
    assert not new_game.is_over()


def test_game_not_over_after_move(new_game):
    game = new_game.apply_move(Move.play(Point(3, 2)))
    assert not game.is_over()


def test_game_over_after_resign(new_game):
    game = new_game.apply_move(Move.resign())
    assert game.is_over()
...
def test_white_wins_when_black_resigns(new_game):
    game = new_game.apply_move(Move.resign())
    assert game.winner() == Player.WHITE


def test_black_wins_with_more_stones(new_game):
    game = new_game.apply_move(Move.play(Point(3, 2)))
    assert game.winner() == Player.BLACK


def test_white_wins_with_more_stones(new_game):
    game = new_game.apply_move(Move.play(Point(3, 2)))
